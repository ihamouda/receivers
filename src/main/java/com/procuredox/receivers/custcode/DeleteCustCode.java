...
public class DeleteCustCode {
    private static final Logger log = LoggerFactory.getLogger(DeleteCustCode.class);
    ...
    public Response deleteCustCode(DeleteCustCodeRequest custCode){
        try{
            sqlConn = mysqlDS.getConnection();
            sqlStmt = sqlConn.prepareStatement("call delete_cust_code(?,?)");
            sqlStmt.setString(1, custCode.getSekkey());
