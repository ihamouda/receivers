...
public class GetCustomers {
    private static final Logger log = LoggerFactory.getLogger(GetCustomers.class);
    ...
    public Response getCustomers(String securityKey){
        try {
            ArrayList<Customer> customers = new ArrayList<>();
            String sqlQuery = "call partners_by_seckey(?)";
            sqlConn = mysqlDS.getConnection();
            sqlStmt = sqlConn.prepareStatement(sqlQuery);
            sqlStmt.setString(1, securityKey);
